...
def predict(X, w, b):
    ...
    for i in range(0, n):
        if Temp[i] >= 0:
            ...
            l.append(-1)
    y = np.asmatrix(np.array(l).reshape((n, 1)))
    #########################################
    return y 
...
def subgradient(x, y, w, b, l=0.001):
    '''
        ...
        Output:
            ...
            dL_db : the subgradient of the bias, a float scalar.
    '''
    #########################################
    ## INSERT YOUR CODE HERE
    f = (w.T)*x + b
...
def train(X, Y, lr = 0.01,C = 1., n_epoch = 10):
    ...
    for _ in range(n_epoch):
        for i in range(n):
            #########################################
            ## INSERT YOUR CODE HERE
            dL_dw, dL_db = subgradient(X[i].T, Y[i], w, b, l)
